    ...
    async function detectLocation() {
      ...
      try {
        ...
        statusIndicator.title = 'Location detection failed. Please select manually.';
   ...
   //detect location as soon as window load
    ...
    async function refreshAllUrls() {
      ...
      const originalFinalUrls = campaigns.map(c => ({ id: c.id, finalUrl: c.finalUrl }));
      ...
      try {
          ...
          const batchPromises = batch.map(async (campaign, index) => {
            ...
            try {
              ...
              const timeoutPromise = new Promise((_, reject) => {
                setTimeout(() => reject(new Error('Individual URL timeout')), 10000); // 10 second timeout per URL
              });
              
              const resolutionPromise = resolveFinalUrl(campaign.url);
              
              // Race between resolution and timeout
              const finalUrl = await Promise.race([resolutionPromise, timeoutPromise]);
              ...
              console.log(`✅ Campaign ${campaignNumber}/${totalCampaigns} resolved successfully`);
              
            } catch (error) {
              console.error(`❌ Campaign ${campaignNumber}/${totalCampaigns} failed:`, error.message);
              
              // Restore original URL if available, otherwise set descriptive error
              const originalData = originalFinalUrls.find(orig => orig.id === campaign.id);
              
              if (originalData && originalData.finalUrl !== 'Loading...' && originalData.finalUrl !== 'Error resolving') {
                campaign.finalUrl = originalData.finalUrl; // Restore previous working URL
                console.log(`🔄 Restored previous URL for campaign ${campaignNumber}`);
              } else {
        ...
        if (errorCount === 0) {
          showNotification(`✅ All ${successCount} URLs refreshed successfully!`, 'success');
        } else if (successCount > 0) {
          showNotification(`⚠️ Refresh completed: ${successCount} successful, ${errorCount} failed`, 'warning');
        } else {
          showNotification(`❌ Refresh failed: All ${errorCount} URLs could not be resolved`, 'error');
        }
    ...
    function renderTable() {
      ...
      campaigns.forEach(c => {
        ...
        const isError = c.finalUrl === 'Error resolving' || 
    ...
    campaigns.sort((a, b) => {
        ...
        if (timeDiff < 1000 && a.originalIndex !== undefined && b.originalIndex !== undefined) {
        return a.originalIndex - b.originalIndex;
        }
    ...
    async function handleFileImport(event) {
      ...
      try {
        ...
        showNotification(`✅ Successfully imported ${data.length} campaigns!`, 'success');
      ...
      return new Promise((resolve, reject) => {
        ...
        reader.onload = function(e) {
          try {
            ...
            if (urlIndex === -1) {
              throw new Error('No URL column found. Please ensure your CSV has a column with "URL", "Link", or "Campaign" in the header.');
            }
            ...
            for (let i = 1; i < lines.length; i++) {
              ...
              if (values.length > urlIndex && values[urlIndex]) {
                ...
                const tags = tagsIndex !== -1 && values[tagsIndex] ? values[tagsIndex].trim() : '';
      ...
      return new Promise((resolve, reject) => {
        ...
        reader.onload = function(e) {
          try {
            ...
            if (urlIndex === -1) {
              throw new Error('No URL column found. Please ensure your Excel file has a column with "URL", "Link", or "Campaign" in the header.');
            }
            ...
            for (let i = 1; i < jsonData.length; i++) {
              ...
              const tags = tagsIndex !== -1 && row[tagsIndex] ? String(row[tagsIndex]).trim() : '';
    ...
    try {
        ...
        // Update progress
        progressNotification.innerHTML = `📊 Processing batch ${Math.floor(i / batchSize) + 1}... (${totalProcessed}/${importedData.length} completed)`;
        ...
        const existingCampaigns = campaigns.filter(c => c.originalIndex === undefined);
        ...
        progressNotification.innerHTML = `✅ Batch ${Math.floor(i / batchSize) + 1} completed! (${totalProcessed}/${importedData.length} total)`;
